...
Route::group(['middleware' => ['auth:api', 'logoutBlocked']], function () {
    JsonApi::register('v1', ['namespace' => 'Api'])->routes(function ($api) {
        ...
        $api->resource('tasks')
            ->only('history')
            ->controller(TaskController::class) //uses the App\Http\Controllers\Api\TaskController
        ...
        $api->resource('users')
            ->only('update-photo')
            ->controller(UserController::class) //uses the App\Http\Controllers\Api\UserController
        ...
        $api->resource('boats')
            ...
            ->controller(BoatController::class)//uses the App\Http\Controllers\Api\BoatController
            ->routes(function ($boats) {
...
//        $api->get('/boats-dashboard', [BoatController::class, 'dashboard'])->name('boats-dashboard');  // tutte le boat di progetti chiusi
        ...
        $api->resource('projects')->only(
            ...
            'closed-projects',
            'change-type')
            ->controller(ProjectController::class) //uses the App\Http\Controllers\Api\ProjectsController
        ...
        // GET - PR28
        $api->resource('projects')->only('get-app-log-structure')
            ->controller(ProjectController::class)
            ->routes(function ($project) {
        ...
        // PR32-a
        $api->resource('projects')->only('generate-application-log-report')
            ->controller(ProjectController::class) //uses the App\Http\Controllers\Api\ProjectController
            ->routes(function ($project) {
        ...
        // PR32-b
        $api->resource('projects')->only('generate-application-log-report-queued')
            ->controller(ProjectController::class) //uses the App\Http\Controllers\Api\ProjectController
            ->routes(function ($project) {
        ...
        $api->resource('projects')->only('download-csv')
            ->controller(ProjectController::class) //uses the App\Http\Controllers\Api\ProjectController
            ->routes(function ($project) {
        ...
        $api->resource('updates')
            ->only('mark-read')
            ->controller(UpdateController::class) // uses the App\Http\Controllers\Api\UpdateController
            ->routes(function ($boats) {
...
//            ->controller(DocumentsController::class) // uses the App\Http\Controllers\Api\DocumentController
        ...
        $api->resource('application-logs')
            ->only('close-remarks')
            ->controller(ApplicationLogController::class) //uses the App\Http\Controllers\Api\ProjectController
            ->routes(function ($project) {
        ...
        $api->resource('histories')
            ...
            ->controller(HistoriesController::class) // uses the App\Http\Controllers\Api\HistoriesController
        ...
        $api->resource('suggestions')
            ->only('update-counter')
            ->controller(SuggestionController::class) // uses the App\Http\Controllers\Api\SuggestionController
